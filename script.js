...
class Portfolio {
    ...
    async loadGitHubProjects() {
        ...
        try {
            ...
            // Add Rip City Ticket Dispatch as featured project first
            const ripCityProject = this.createRipCityProject();
            projectsGrid.appendChild(ripCityProject);
            
            // Replace 'j-mazz' with your actual GitHub username
            const response = await fetch('https://api.github.com/users/j-mazz/repos?sort=updated&per_page=5');
            ...
            if (filteredRepos.length === 0) {
                // Keep Rip City project and add empty state message
                const emptyDiv = document.createElement('div');
                emptyDiv.className = 'col-span-full text-center';
                emptyDiv.innerHTML = '<p>Additional GitHub projects loading...</p>';
            ...
            // Keep Rip City project even if GitHub API fails
            if (projectsGrid.children.length === 1) {
                const errorDiv = document.createElement('div');
                errorDiv.className = 'col-span-full text-center';
                errorDiv.innerHTML = '<p>Additional projects loading...</p>';
                projectsGrid.appendChild(errorDiv);
            }
        }
    }

    createRipCityProject() {
        const card = document.createElement('div');
        card.className = 'project-card featured-project';
        
        card.innerHTML = `
            <div class="project-header">
                <h3 class="project-title">🏀 Rip City Ticket Dispatch</h3>
                <div class="project-links">
                    <a href="https://github.com/J-mazz/rip-city-ticket-dispatch" target="_blank" rel="noopener" class="project-link" title="View on GitHub">
                    ...
                    <a href="/rip-city-tickets" class="project-link" title="Live Demo">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            ...
            <p class="project-description">🎫 Trail Blazers-themed automated ticket monitoring system for Portland sports & music events. Real-time deal scanning, smart price alerts, and beautiful responsive React interface.</p>
