...
if (process.env.OPENAI_API_KEY) {
  ...
  console.warn('⚠️ OPENAI_API_KEY not set. Odds generation will be disabled.');
}
...
async function getOddsForEvent(event) {
  // If OpenAI is not configured, return mock odds for demo purposes
  if (!openai) {
    console.log('Generating mock odds since OpenAI is not configured');
    const teams = event.teams || [];
    if (teams.length === 2) {
      // Return mock odds that add up to 100
      return {
        [teams[0]]: 55,
        [teams[1]]: 45
      };
