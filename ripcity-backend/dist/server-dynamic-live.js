...
app.get('/api/blazers', async (req, res) => {
    try {
        logger_1.default.info('Fetching Trail Blazers events from live API');
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        logger_1.default.error('Blazers API error', { error: errorMessage });
        res.status(500).json({
            success: false,
...
app.get('/api/deals/hot', async (req, res) => {
    try {
        logger_1.default.info('Generating hot deals from live API data');
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        logger_1.default.error('Hot deals API error', { error: errorMessage });
        res.status(500).json({
            success: false,
...
app.get('/api/deals/free', async (req, res) => {
    try {
        ...
        const freeEvents = await eventAggregation_1.default.getFreeEvents(30);
        res.json({
            success: true,
            deals: freeEvents,
            source: 'live_api_aggregation',
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        logger_1.default.error('Free events API error', { error: errorMessage });
        res.status(500).json({
            success: false,
...
app.get('/api/deals/search', async (req, res) => {
    try {
        ...
        logger_1.default.info('Searching events in live APIs', { query: q });
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        logger_1.default.error('Search API error', { error: errorMessage });
        res.status(500).json({
            success: false,
...
app.get('/api/premium/deals', auth_1.authenticateToken, (0, auth_1.requireSubscription)('premium'), async (req, res) => {
    try {
        ...
        const aggregatedDeals = await eventAggregation_1.default.searchAllEvents({
            category: 'all'
        });
...
const server = app.listen(PORT, '0.0.0.0', async () => {
    logger_1.default.info(`🚀 Rip City DYNAMIC Server running on port ${PORT}`);
    ...
    logger_1.default.info(`🤖 Server Type: DYNAMIC (Real API calls, not static mock data)`);
