...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // Parse sources
    const sourcesArray = (sources as string).split(',').map(s => s.trim()) as ('ticketmaster' | 'eventbrite')[];
...
router.get('/hot', async (req: Request, res: Response) => {
  try {
    const hotDeals = await eventAggregationService.getHotDeals(10);
...
router.get('/free', async (req: Request, res: Response) => {
  try {
    ...
    const freeEvents = await eventAggregationService.getFreeEvents(parseInt(limit as string));
