...
app.get('/api/venues', (req, res) => {
  logger.info('Demo: Serving mock venues data');
  res.json({
    success: true,
    venues: mockVenues,
    source: 'demo_data',
...
app.get('/api/events', (req, res) => {
  ...
  logger.info('Demo: Serving mock events data', { category, limit });
  ...
  const limitedEvents = filteredEvents.slice(0, parseInt(limit as string || '50'));
...
app.get('/api/blazers', (req, res) => {
  logger.info('Demo: Serving mock Blazers events');
...
app.get('/api/deals/hot', (req, res) => {
  logger.info('Demo: Serving mock hot deals');
...
app.get('/api/deals/free', (req, res) => {
  logger.info('Demo: Serving mock free events');
...
app.get('/api/deals/search', (req, res) => {
  ...
  logger.info('Demo: Searching mock events', { query: q });
  const searchResults = mockEvents.filter(event => 
    event.name.toLowerCase().includes((q as string).toLowerCase()) ||
    event.venue.toLowerCase().includes((q as string).toLowerCase()) ||
    event.category.toLowerCase().includes((q as string).toLowerCase())
  );
...
const server = app.listen(PORT, '0.0.0.0', () => {
  logger.info(`🎭 Rip City DEMO Server running on port ${PORT}`);
  logger.info(`🏀 Environment: DEMO MODE`);
  logger.info(`🎪 Data: Mock data for demonstration`);
  logger.info(`🔒 APIs: No live API calls made`);
  logger.info(`✅ Status: Safe for testing and demos`);
  logger.info(`🌐 Access: http://localhost:${PORT}`);
});

// Graceful shutdown
process.on('SIGTERM', () => {
  logger.info('SIGTERM received, shutting down demo server gracefully');
  server.close(() => {
    process.exit(0);
  });
});
...
process.on('SIGINT', () => {
  logger.info('SIGINT received, shutting down demo server gracefully');
  server.close(() => {
    process.exit(0);
  });
});
