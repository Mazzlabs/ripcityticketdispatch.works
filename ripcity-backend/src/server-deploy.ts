...
app.get('/health', async (req, res) => {
  try {
    ...
    try {
      ...
      logger.warn('Ticketmaster API check failed in deployment:', e);
    }
    ...
    try {
      ...
      logger.warn('Eventbrite API check failed in deployment:', e);
    }
    ...
    if (missingEnvVars.length > 0) {
      logger.error('Deployment health check failed - missing environment variables:', missingEnvVars);
      return res.status(503).json(healthStatus);
    }
    
    res.json(healthStatus);
  } catch (error) {
    logger.error('Deployment health check failed:', error);
    res.status(500).json({
      status: 'error',
...
app.get('/api/venues', async (req, res) => {
  try {
    logger.info('Deployment: Fetching venues from live Ticketmaster API');
    ...
    logger.error('Deployment Ticketmaster API error', { error: errorMessage, environment: NODE_ENV });
    res.status(500).json({
      success: false,
...
app.get('/api/events', async (req, res) => {
  try {
    ...
    logger.info('Deployment: Fetching events from live APIs', { category, limit, environment: NODE_ENV });
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    logger.error('Deployment API aggregation error', { error: errorMessage, environment: NODE_ENV });
    res.status(500).json({
      success: false,
...
app.get('/api/blazers', async (req, res) => {
  try {
    logger.info('Deployment: Fetching Trail Blazers events', { environment: NODE_ENV });
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    logger.error('Deployment Blazers API error', { error: errorMessage, environment: NODE_ENV });
    res.status(500).json({
      success: false,
...
app.get('/api/deals/hot', async (req, res) => {
  try {
    logger.info('Deployment: Generating hot deals', { environment: NODE_ENV });
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    logger.error('Deployment hot deals error', { error: errorMessage, environment: NODE_ENV });
    res.status(500).json({
      success: false,
...
app.use((error: Error, req: express.Request, res: express.Response, next: express.NextFunction) => {
  logger.error('Deployment server error', { 
    error: error.message, 
    stack: error.stack,
    path: req.path,
    method: req.method,
...
const server = app.listen(PORT, '0.0.0.0', async () => {
  logger.info(`🚀 Rip City DEPLOYMENT Server running on port ${PORT}`);
  ...
  logger.info(`💳 Stripe: BYPASSED FOR MVP`);
  logger.info(`📱 Twilio SMS: BYPASSED FOR MVP`);
  logger.info(`📧 SendGrid: BYPASSED FOR MVP`);
  logger.info(`🔧 Server Type: DEPLOYMENT (Environment validation & security)`);
...
process.on('SIGTERM', async () => {
  logger.info('SIGTERM received, shutting down deployment server gracefully');
  server.close(async () => {
...
process.on('SIGINT', async () => {
  logger.info('SIGINT received, shutting down deployment server gracefully');
  server.close(async () => {
    await MongoDB.disconnect();
    process.exit(0);
  });
