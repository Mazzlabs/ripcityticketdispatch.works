...
export interface Deal {
  ...
  dealScore: number; // 0-100, higher is better
  confidence: number; // 0-100, how confident we are this is a good deal
...
export interface DealScoringCriteria {
  minSavingsPercentage: number; // Minimum % savings to consider it a deal
  maxPriceThreshold?: number; // Don't flag expensive tickets as deals
  venuePopularity: number; // 1-10, more popular venues get higher scores
  eventPopularity: number; // 1-10, more popular events get higher scores
  timeUntilEvent: number; // Days until event
  historicalPricing?: {
    averagePrice: number;
    lowestSeen: number;
    highestSeen: number;
...
export class DealScoringService {
  private readonly HOT_DEAL_THRESHOLD = 85;
  private readonly WARM_DEAL_THRESHOLD = 70;
  private readonly GOOD_DEAL_THRESHOLD = 55;
  ...
  scoreDeals(events: TicketmasterEvent[], criteria?: Partial<DealScoringCriteria>): Deal[] {
    const defaultCriteria: DealScoringCriteria = {
  ...
  private calculateDealScore(params: {
    savingsPercentage: number;
    currentPrice: number;
  ...
  private calculateConfidence(savingsPercentage: number, criteria: DealScoringCriteria): number {
    let confidence = 50; // Base confidence

    // Higher savings = higher confidence
    if (savingsPercentage >= 30) confidence += 30;
    else if (savingsPercentage >= 20) confidence += 20;
    else if (savingsPercentage >= 15) confidence += 15;
    else confidence += savingsPercentage;
  ...
  private categorizeEvent(segmentName?: string): Deal['category'] {
    if (!segmentName) return 'other';
    
    const segment = segmentName.toLowerCase();
    if (segment.includes('sport')) return 'sports';
    if (segment.includes('music') || segment.includes('concert')) return 'music';
    if (segment.includes('theatre') || segment.includes('theater')) return 'theater';
    if (segment.includes('family') || segment.includes('kids')) return 'family';
    
    return 'other';
  }

  private getAlertLevel(dealScore: number): Deal['alertLevel'] {
    if (dealScore >= this.HOT_DEAL_THRESHOLD) return 'hot';
    if (dealScore >= this.WARM_DEAL_THRESHOLD) return 'warm';
    if (dealScore >= this.GOOD_DEAL_THRESHOLD) return 'good';
    return 'normal';
  }

  private generateTags(event: TicketmasterEvent, savingsPercentage: number): string[] {
    const tags: string[] = [];

    // Savings-based tags
    if (savingsPercentage >= 30) tags.push('HUGE SAVINGS');
    else if (savingsPercentage >= 20) tags.push('GREAT DEAL');
    else if (savingsPercentage >= 15) tags.push('GOOD DEAL');
    ...
    if (venue?.includes('moda center')) tags.push('MODA CENTER');
    if (venue?.includes('providence park')) tags.push('PROVIDENCE PARK');

    // Time-based tags
    const eventDate = new Date(event.dates.start.dateTime || event.dates.start.localDate);
    const daysUntil = Math.ceil((eventDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
    if (daysUntil <= 3) tags.push('LAST MINUTE');
    else if (daysUntil <= 7) tags.push('THIS WEEK');
  ...
  filterDeals(deals: Deal[], filters: {
    ...
    return deals.filter(deal => {
      ...
      if (filters.venues && !filters.venues.some(venue => 
   ...
   */
  sortDeals(deals: Deal[], sortBy: 'score' | 'savings' | 'price' | 'date' = 'score'): Deal[] {
